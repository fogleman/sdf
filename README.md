...
```
Note: PIL ImageFont, which is used to load fonts, is only searching for the font by name on Windows.
On other systems the full path to the font has to be provided.
(e.g. on Ubuntu `/usr/share/fonts/truetype/freefont/FreeMono.ttf` or `/usr/share/fonts/truetype/roboto/unhinted/RobotoCondensed-Regular.ttf` etc)
